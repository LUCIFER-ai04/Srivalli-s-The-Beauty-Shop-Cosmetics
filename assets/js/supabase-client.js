...
(function () {
  ...
  async function createOrder(order) {
    if (!connected) {
      ...
      return { id:'LOCAL-'+Date.now(), order_number: order.order_number||'PENDING-SETUP', saved:false };
    }
    // Use plain insert (no .select()) to avoid the INSERT...RETURNING RLS issue
    // for guest/anon users where auth.uid()=null fails the SELECT policy check.
    // The order_number is already set client-side so we don't need the DB value back.
    var r = await client.from('orders').insert(order);
    if (r.error) throw r.error;
    return Object.assign({}, order, { saved: true });
  }
  ...
  async function getProductImages(productId) {
    if (!connected) return [];
    try {
      var r = await client.from('product_images').select('*')
        .eq('product_id', String(productId)).order('sort_order');
      return r.data || [];
    } catch(e) { return []; }
  }
  async function addProductImage(productId, imageUrl, sortOrder) {
    if (!connected) return null;
    var r = await client.from('product_images').insert({
      product_id: String(productId), image_url: imageUrl, sort_order: sortOrder || 0
    });
    if (r.error) throw r.error;
    return r.data;
  }
  async function deleteProductImage(id) {
    if (!connected) return;
    await client.from('product_images').delete().eq('id', id);
  }
  async function uploadProductImage(productId, file) {
    if (!connected) return null;
    var ext = (file.name.split('.').pop() || 'jpg').toLowerCase();
    var path = 'products/' + productId + '/' + Date.now() + '.' + ext;
    var r = await client.storage.from('product-images').upload(path, file, {upsert:true});
    if (r.error) throw r.error;
  ...
  async function adminDeleteProduct(itemCode) {
    // Insert into hidden_products table — get_modified_products() returns
    // hidden items as active:false, which app.js filters from the shop display
    if (!connected) throw new Error('Not connected.');
    var r = await client.from('hidden_products')
      .upsert({ item_code: itemCode }, { onConflict: 'item_code' });
    if (r.error) throw r.error;
  }

  async function adminRestoreProduct(itemCode) {
    // Remove from hidden_products to restore a previously deleted product
    if (!connected) throw new Error('Not connected.');
    var r = await client.from('hidden_products').delete().eq('item_code', itemCode);
    if (r.error) throw r.error;
  }
  ...
  window.db = {
    ...
    adminRestoreProduct: adminRestoreProduct,
