...
(function () {
  ...
  /* ---------------------------------------------------------------- GATE -- */
  /* Inline login shown when not authenticated or not admin */
  function showAdminLoginUI(msg) {
    ...
    gate.innerHTML =
      '<div style="display:flex;align-items:center;justify-content:center;min-height:70vh;">'+
        '<div style="background:#fff;border:1px solid var(--cream-deep);border-radius:var(--radius);'+
          'padding:40px 32px;max-width:380px;width:90%;box-shadow:0 4px 24px rgba(26,16,21,.12);text-align:center;">'+
          '<div style="font-size:2.2rem;margin-bottom:10px;">&#128274;</div>'+
          '<h2 style="font-size:1.2rem;margin-bottom:6px;">Admin Area</h2>'+
          '<p style="font-size:.82rem;color:var(--ink-soft);margin-bottom:4px;">Private &#8212; authorised accounts only.</p>'+
          (msg ? '<div style="background:#fef3c7;border:1px solid #fcd34d;border-radius:6px;padding:8px 12px;'+
            'font-size:.8rem;color:#92400e;margin:8px 0 14px;">'+msg+'</div>' : '<div style="height:14px;"></div>')+
          '<button id="adminInlineGoogleBtn" style="width:100%;display:flex;align-items:center;justify-content:center;'+
            'gap:10px;padding:13px 16px;border:1.5px solid #ddd;border-radius:8px;background:#fff;'+
            'cursor:pointer;font-size:.95rem;font-weight:600;margin-bottom:12px;">'+
          ...
          '</button>'+
          '<div style="font-size:.7rem;color:var(--ink-soft);">Authorised admin accounts only.</div>'+
        '</div>'+
      '</div>';
    var btn = document.getElementById("adminInlineGoogleBtn");
    if (!btn) return;
    btn.addEventListener("click", async function() {
      btn.disabled=true; btn.style.opacity=".6"; btn.textContent="Signing in...";
      try {
        ...
        showAdminLoginUI("Error: " + shared.escapeHtml((e&&e.message)||String(e)));
      }
    });
  }
    ...
    try {
      ...
      if (bar && session.user) {
        ...
        if (soBtn) soBtn.addEventListener("click", function(e) {
          e.preventDefault();
          db.signOut().then(function(){ showAdminLoginUI(); }).catch(function(){ showAdminLoginUI(); });
        });
      }
      return true;
    } catch(e) {
    ...
    var rows = list.slice(0, 300).map(function (p) {
      ...
      return '<tr>' +
        ...
        '<td style="display:flex;gap:6px;padding:8px 0;">' +
          ...
          '<button class="btn btn-sm del-product-btn" data-id="' + (p.item_code||p.id) + '" style="background:#fee2e2;color:#991b1b;border:1px solid #fca5a5;">\uD83D\uDDD1\uFE0F Delete</button>' +
        '</td>' +
        '</tr>';
    }).join("");
  ...
  function openProductModal(p) {
    ...
    openModal('<div style="display:grid;grid-template-columns:200px 1fr;gap:20px;align-items:start;">' +
      ...
      '<div>' +
        ...
        '<div style="display:flex;gap:10px;flex-wrap:wrap;margin-top:14px;">' +
          ...
          (p.id ? '<button class="btn btn-outline" id="mPhotosBtn" style="flex:0 0 auto;">&#128247; Photos</button>' : '') +
    ...
    var photosBtn = document.getElementById("mPhotosBtn");
    if (photosBtn) photosBtn.addEventListener("click", function() { closeModal(); openProductImagesModal(p); });
  ...
  async function openProductImagesModal(p) {
    var imgs = [];
    try { imgs = await db.getProductImages(p.id); } catch(e){}
    function renderImgModal() {
      openModal(
        '<h3 style="margin-bottom:12px;">Product Photos</h3>'+
        '<p style="font-size:.8rem;color:var(--ink-soft);margin-bottom:10px;">First photo shows on shop. Visitors swipe right on product page to see all.</p>'+
        '<div id="imgList" style="display:grid;grid-template-columns:repeat(3,1fr);gap:8px;margin-bottom:14px;">'+
          imgs.map(function(img){
            return '<div style="position:relative;aspect-ratio:1;border-radius:6px;overflow:hidden;border:1px solid var(--cream-deep);">'+
              '<img src="'+shared.escapeHtml(img.image_url)+'" style="width:100%;height:100%;object-fit:cover;" loading="lazy">'+
              '<button class="del-img-btn" data-id="'+img.id+'" style="position:absolute;top:4px;right:4px;background:rgba(0,0,0,.6);color:#fff;border:none;border-radius:50%;width:24px;height:24px;cursor:pointer;font-size:.8rem;line-height:1;">&times;</button>'+
            '</div>';
          }).join("")+
        '</div>'+
        '<div style="border:2px dashed var(--cream-deep);border-radius:8px;padding:14px;text-align:center;">'+
          '<input type="file" id="imgFileInput" accept="image/*" multiple style="display:none;">'+
          '<button id="imgUploadBtn" class="btn btn-primary" style="margin-bottom:8px;">Upload Photos</button>'+
          '<div id="imgUploadStatus" style="font-size:.8rem;min-height:16px;"></div>'+
        '</div>'+
        '<div style="display:flex;justify-content:flex-end;margin-top:14px;">'+
          '<button class="btn btn-outline" id="imgDoneBtn">Done</button>'+
        '</div>', true);
      document.getElementById("imgDoneBtn").addEventListener("click", closeModal);
      document.getElementById("imgUploadBtn").addEventListener("click", function(){ document.getElementById("imgFileInput").click(); });
      document.getElementById("imgFileInput").addEventListener("change", async function() {
        var files = Array.from(this.files||[]);
        if (!files.length) return;
        var status=document.getElementById("imgUploadStatus"), btn=document.getElementById("imgUploadBtn");
        btn.disabled=true; btn.textContent="Uploading...";
        var done=0;
        for (var i=0;i<files.length;i++) {
          try {
            status.textContent="Uploading "+(i+1)+" of "+files.length+"...";
            var url=await db.uploadProductImage(p.id,files[i]);
            await db.addProductImage(p.id,url,imgs.length+i);
            done++;
          } catch(e){ status.textContent="Error: "+(e.message||e); }
        }
        status.innerHTML='<span style="color:var(--success);">'+done+' photo'+(done===1?"":"s")+' added!</span>';
        try{ imgs=await db.getProductImages(p.id); }catch(e){}
        setTimeout(renderImgModal,800);
      });
      document.querySelectorAll(".del-img-btn").forEach(function(btn){
        btn.addEventListener("click", async function(){
          if (!confirm("Remove this photo?")) return;
          btn.disabled=true;
          await db.deleteProductImage(btn.dataset.id);
          try{ imgs=await db.getProductImages(p.id); }catch(e){}
          renderImgModal();
        });
      });
